...
The independence assumption is violated because there may be many other confounding variables including how healthy the baby is, etc.
...
**THERE IS AN ERROR TERM IN THIS LINK??**
There is no random variable term in the given equation because the equation does not represent the statistical model, but represents the formula used by the R to build the linear regression model. We can see that the statistical model does in fact include an error term instead.
