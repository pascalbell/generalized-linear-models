...
> Where $y_{ij}$ is the temperature of the jth observation of the ith group and $x_i$ is the mean cricket chirps for the ith group.
...
> **THERE IS AN ERROR TERM IN THIS LINK??** There is no random variable term in the given equation because the equation does not represent the statistical model, but represents the formula used by the R to build the linear regression model. We can see that the statistical model does in fact include an error term instead.
...
> `logprice` of a house is expected to increase by .0399 **UNIT??** when `sqft` increases by 100
