...
**DOUBLE CHECK THE LIKELIHOOD FUNCTION LAST PART**
...
$H_0:p_{B(conditional)} = p_{B|B} = p_{B (unconditional)}$
$H_a:$ At least one of $p_{B(conditional)}, p_{B|B}, p_{B(unconditional)}$ differs from the others.

Let's test the null hypotheses:

MLE for no hot hand model:
$\hat{p_B} = \frac{10}{25} = 0.4$

MLEs for hot hand model:
$\hat{p_B} = \frac{7}{18} = 0.39$
$\hat{p_{B|B}} = \frac{3}{7} = 0.43$
...
The differences in the maximum likelihood is approximately -0.016.

The LRT is -0.03292466.

The LRT statistic follows a chi squared distribution with 1 degree of freedom. Therefore, the p-value is $P(X^2 > LRT)$ The p-value is very large, so we fail to reject the null hypothesis. We do not find evidence that the conditional model is an improvement over the unconditional model, and we do not find significant evidence that hot hand exists.

Below is the code that calculates all the variables used above.

**DOUBLE CHECK THIS P VALUE AND ALL**
...
diff <- max_log_lik_no_hot - max_log_lik_hot
